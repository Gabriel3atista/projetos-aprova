<template>
    <ul class="grid gap-4" :class="pacotes.length > 1 ? 'grid-cols-2 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1'">
        <li v-for="(pacote, index) in pacotes" :key="index"
            ...
            <div class="flex flex-col items-center justify-between text-center h-full">
                <p v-if="pacote.attributes.descricao.split('-')[1]" class="uppercase font-bold text-white">{{
                ...
                <p v-else class="flex uppercase font-bold text-white">
                    ...
                    <i @click="toggleCursoInfo(index, pacote.id)"
                        class="info absolute z-10 cursor-pointer top-1 right-1 text-white/50 hover:mainTextColor">
                ...
                <MountPrice class="mt-6" :price="pacote.attributes.valor"
                    :discount="pacote.attributes.valor_desconto" />
                <div class="flex flex-col items-center justify-center mt-6">
                    <button @click="compraCurso(pacote.id)"
                        class="button py-2 px-4 text-white font-bold rounded uppercase cursor-pointer">comprar</button>
                    <a :href="`https://www.aprovaconcursos.com.br/concurso/${iniciais}/${idInstituicao}/${urlCargo(pacote.attributes.cargo)}/${pacote.id}`"
                        target="_blank" rel="noopener noreferer" class="text-white/50 underline mt-3">saiba mais</a>
                </div>
            </div>
            <div v-if="pacote.id == 129880" class="cursoInfo absolute inset-0 text-white/50 w-full h-full hidden">
                <div class="darkBgColor p-3 pt-12 h-full rounded">STJ, STF, STM, TCU, TSE, TRF’s, TJ’s, TST’s, TRT’s,
                    TCE’s, TCM’s e TRE’s, MP’s, AL’s</div>
            </div>
            <div v-if="pacote.id == 130020" class="cursoInfo absolute inset-0 text-white/50 w-full h-full hidden">
                <div class="darkBgColor p-3 pt-12 h-full rounded">Ministérios Públicos, Tribunais, AGU, Secretarias da
                    Fazenda, Assembleias Legislativas, Controladorias, Bancos, Conselhos Regionais, Universidades,
                    Câmaras e Prefeituras Municipais</div>
            </div>
            <div v-if="pacote.id == 130019" class="cursoInfo absolute inset-0 text-white/50 w-full h-full hidden">
                <div class="darkBgColor p-3 pt-12 h-full rounded">Ministério da Saúde, ANS, ANVISA, Secretarias,
                    Institutos, Fundações, Consórcios Intermunicipais, Conselhos Regionais e Hospitais Públicos</div>
            </div>
            <div v-if="pacote.id == 130030" class="cursoInfo absolute inset-0 text-white/50 w-full h-full hidden">
                <div class="darkBgColor p-3 pt-12 h-full rounded">INSS, Receita Federal, Ministérios, IBGE, ABIN, DNIT,
                    IBAMA, DETRAN’s, Correios, PF, PRF, PC’s, PM’s, Bancos, Forças Armadas, Câmaras, Prefeituras e
                    muitos outros.</div>
            </div>
            <div v-if="pacote.id == 130021" class="cursoInfo absolute inset-0 text-white/50 w-full h-full hidden">
                <div class="darkBgColor p-3 pt-12 h-full rounded">Universidades, Faculdades, Institutos Federais,
                    Secretarias de Educação, Câmaras e Prefeituras Municipais</div>
            </div>
        </li>
    </ul>
</template>
