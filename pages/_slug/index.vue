<template>
  <div
    :style="{
      ...
      '--second-color-hover': secondColorHover,
      '--dark-bg-color': darkBgColor,
    }"
  >
    <header
      v-if="
        projects[slug].iniciais == 'oab' ||
        projects[slug].iniciais == 'cfc' ||
        projects[slug].iniciais == 'enem' ||
    ...
    >
      ...
      >
        <!-- Button -->
        <div
          class="absolute flex items-center justify-center w-12 h-full top-0 left-0 cursor-pointer z-20 lg:hidden"
          @click="toggleMenu"
        >
        ...
        <!-- Logo -->
        <div
          class="flex items-center relative w-[200px] h-full z-20 lg:justify-start"
        >
          <nuxt-link to="/">
            <img
              class="absolute top-1/2 transform -translate-y-1/2 max-h-full"
              :src="require(`~/assets/images/${logoImg}`)"
              :alt="projects[slug].assets.logoAlt"
              width="187"
              height="33"
            />
          </nuxt-link>
        ...
        <!-- Menu -->
        <div
          class="absolute flex flex-col w-full h-full top-0 px-6 z-10 duration-500 ease-in-expo overflow-hidden bg-white lg:justify-between lg:relative lg:flex lg:flex-row lg:items-center lg:w-auto lg:px-0 lg:pb-0 lg:overflow-visible"
          :class="
            active ? 'h-[100vh]' : 'pointer-events-none lg:pointer-events-auto'
          "
        >
          <ul
            class="menuList uppercase text-xl font-semibold pt-28 transform duration-700 ease-in-out divide-y divide-gray/[.20] lg:divide-y-0 lg:flex lg:items-center lg:content-center lg:text-sm lg:pt-0 lg:h-full"
            :class="
              active
                ? 'opacity-100 translate-y-0'
                : 'opacity-0 -translate-y-8 lg:opacity-100 lg:-translate-y-0'
            "
          >
            <li
              v-for="(item, index) in projects[slug].menu"
              :key="index"
              class="cursor-pointer border-y border-gray-black lg:h-12 lg:flex lg:items-center lg:py-0 lg:px-4 lg:border-0"
            >
              <a :href="item.url" class="w-full block p-4">{{ item.title }}</a>
            </li>
            <li
              class="group cursor-pointer p-4 lg:h-12 lg:flex lg:items-center lg:py-0 lg:px-4"
            >
              <div class="flex justify-between lg:justify-center">
                <p class="lg:mr-1">contato</p>
                <svg
                  class="w-7 text-gray fill-current transform transition duration-500 rotate-90 group-hover:-rotate-90 lg:w-6"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                >
              ...
              <div
                class="h-0 overflow-hidden normal-case text-base font-normal transform transition duration-200 group-hover:h-auto group-hover:pt-4 lg:group-hover:pt-2 lg:absolute lg:top-12 lg:right-0 lg:w-auto"
              >
          ...
          <button
            ...
            :class="
              active
                ? 'translate-y-0'
                : '-translate-y-8 lg:opacity-300 lg:-translate-y-0'
            "
          >
            <a
              :href="urls.ava"
    ...
    >
      ...
      >
        ...
        <!-- Telefone -->
        <div
          class="flex justify-center items-center relative w-[200px] h-full z-20 lg:justify-start"
        >
          <a
            :href="`tel:${telefone.replaceAll(' ', '')}`"
            class="font-bold secondTextColor"
            >{{ telefone }}</a
          >
        </div>
        <!-- Menu -->
        <div
          class="darkBgColor absolute flex flex-col w-full h-full top-0 px-6 z-10 duration-500 ease-in-expo overflow-hidden lg:justify-between lg:relative lg:flex lg:flex-row lg:items-center lg:w-auto lg:px-0 lg:pb-0 lg:overflow-visible"
          :class="
            active ? 'h-[100vh]' : 'pointer-events-none lg:pointer-events-auto'
          "
        >
          <ul
            class="menuList text-base-white uppercase text-xl font-semibold pt-28 transform duration-700 ease-in-out divide-y divide-gray/[.20] lg:divide-y-0 lg:flex lg:items-center lg:content-center lg:text-sm lg:pt-0 lg:h-full"
            :class="
              active
                ? 'opacity-100 translate-y-0'
                : 'opacity-0 -translate-y-8 lg:opacity-100 lg:-translate-y-0'
          ...
          >
            <li
              v-for="(item, index) in projects[slug].menu"
              :key="index"
              class="cursor-pointer text-base-white border-y border-gray-black lg:h-12 lg:flex lg:items-center lg:py-0 lg:px-4 lg:border-0"
            >
              <a :href="item.url" class="w-full block p-4">{{ item.title }}</a>
            </li>
            <li
            ...
            >
              <div class="flex justify-between lg:justify-center">
                <p class="lg:mr-1">contato</p>
                <svg
                  class="w-7 text-base-white fill-current transform transition duration-500 rotate-90 group-hover:-rotate-90 lg:w-6"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                >
              ...
              >
                <ul
                  class="subMenuList text-base-white darkBgColor lg:p-4 lg:rounded"
                >
                  <li class="ounded hover:bg-white/[.07]">
                    <span
                      href="#"
                      class="flex p-3 w-full"
                      @click="openClickToCall"
                    >
                      <svg
                        class="mr-3 fill-current"
                        width="24"
    ...
    <!-- Click to call -->
    <div
      v-if="clickToCallActive"
      class="fixed flex z-50 justify-center items-center w-full h-full bg-black/70 backdrop-blur-sm px-6"
    >
      <div
        class="click-to-call relative w-full h-full rounded-lg bg-white overflow-hidden"
      >
        <span
          class="cursor-pointer absolute top-0 right-0 z-20 w-10 h-10 p-1 flex justify-center items-center"
        ...
        >
          <div class="w-20 h-20">
            <svg
              v-if="clickToCallStatus == 'default'"
            ...
            <svg
              v-if="clickToCallStatus == 'running'"
              class="w-full h-auto text-white"
              width="24"
              height="24"
              viewBox="0 0 24 24"
            >
              <path
                d="M16.793 5.793L12.293 10.293L13.707 11.707L18.207 7.207L21 10V3H14L16.793 5.793Z"
                fill="currentColor"
              />
              <path
            ...
            <svg
              v-if="clickToCallStatus == 'success'"
              class="w-full h-auto text-white"
              width="24"
              height="24"
              viewBox="0 0 24 24"
        ...
        <div class="p-6">
          <h2 class="mainTextColor">
            Está com alguma dúvida? Preencha abaixo seu telefone e ligaremos pra
            você:
          </h2>
          <form action="" class="mt-6" @submit.prevent="sendNumber" required>
            <div class="w-full">
              <input
                type="text"
                v-mask="['(##) ####-####', '(##) #####-####']"
                v-model="celularClickToCall"
                placeholder="Preencha seu celular ou telefone"
                class="w-full h-12 pl-4 border-b border-black/25 focus-visible:outline-none"
                required
              />
            </div>
            <div class="mt-6">
              <label class="text-sm" for="privacidadeClick">
                <input
                  type="checkbox"
                  name="privacidadeClick"
                  id="privacidadeClick"
                  v-model="termos"
                  required
                />
                Autorizo a coleta e tratamentos dos dados pessoais fornecidos,
                por mim titularizados, que servirão de base para o contato e
                comunicação com o Fale Conosco Aprova. O dado pessoal
                disponibilizado não terá uso que exorbite a tentativa de 3
                telefonemas. Central exclusiva para dúvidas referentes à
                matrícula. Se você já é aluno ligue para
                <a
                  :href="`tel:${telefone.replaceAll(' ', '')}`"
                  class="font-bold"
                  >{{ telefone }}</a
            ...
            <div class="mt-6">
              <button
                ...
                :class="
                  termos ? 'opacity-100' : 'opacity-50 pointer-events-none'
                "
              >
                enviar
              </button>
          ...
          <div
            class="callbox absolute flex-col justify-center p-6 inset-0 bg-white callback-box no-transition"
            style="display: none"
          >
            <span class="message">
    ...
    <LazyHydrate when-idle>
      <div>
        <HomeOab
          v-if="projects[slug].iniciais == 'oab'"
          :preparatorio="preparatorio"
          :iniciais="projects[slug].iniciais"
        />
        <HomeCfc
          v-if="projects[slug].iniciais == 'cfc'"
          :preparatorio="preparatorio"
          :iniciais="projects[slug].iniciais"
        />
        ...
        <HomeCertificacoesFinanceiras
          v-if="projects[slug].iniciais == 'certificacoes-financeiras'"
          :idInstituicao="projects[slug].id"
          :editais="editais"
          :iniciais="projects[slug].iniciais"
        />
        <HomeCertificacoesTi
          v-if="projects[slug].iniciais == 'certificacoes-ti'"
          :idInstituicao="projects[slug].id"
          :editais="editais"
          :iniciais="projects[slug].iniciais"
        />
        <HomeCertificacoesProjetos
          v-if="projects[slug].iniciais == 'certificacoes-projetos'"
          :idInstituicao="projects[slug].id"
          :editais="editais"
          :iniciais="projects[slug].iniciais"
        />
        <HomePrimeiroConcurso
          v-if="projects[slug].iniciais == 'cursos-por-area-de-interesse'"
          :idInstituicao="projects[slug].id"
          :editais="editais"
          :iniciais="projects[slug].iniciais"
        />
    ...
    <LazyHydrate when-visible>
      <div>
        ...
        <Questoes
          v-if="projects[slug].iniciais == 'assinatura'"
          :linkAprovaQuestoes="urls.aprovaQuestoes"
        />
        <Packages
          v-if="
            preparatorios.length > 1 &&
            projects[slug].iniciais != 'assinatura' &&
            projects[slug].iniciais != 'certificacoes-financeiras' &&
            projects[slug].iniciais != 'cursos-por-area-de-interesse' &&
            projects[slug].iniciais != 'certificacoes-ti' &&
...
export default {
  ...
  components: {
    ...
    HomeCertificacoesFinanceiras: () =>
      import("@/components/HomeCertificacoesFinanceiras.vue"),
    HomeCertificacoesTi: () => import("@/components/HomeCertificacoesTi.vue"),
    HomeCertificacoesProjetos: () =>
      import("@/components/HomeCertificacoesProjetos.vue"),
  ...
  head() {
    return {
      ...
      link: [
        ...
        {
          rel: "canonical",
          href: `https://www.aprovaconcursos.com.br/${
            this.projects[this.slug].canonical
          }`,
        },
      ],
      meta: [
        {
          name: "theme-color",
          content:
            this.projects[this.slug].iniciais == "certificacoes-projetos" ||
            this.projects[this.slug].iniciais == "certificacoes-financeiras" ||
            this.projects[this.slug].iniciais == "certificacoes-ti" ||
            this.projects[this.slug].iniciais == "cursos-por-area-de-interesse"
              ? "#181A1D"
              : "#FFFFFF",
        },
      ],
    ...
    return {
      ...
      urls: {
        ...
        politicaDePrivacidade:
          "https://www.aprovaconcursos.com.br/politica-de-privacidade",
        politicaDeCancelamento:
          "https://www.aprovaconcursos.com.br/politica-de-cancelamento",
  ...
  async asyncData({ params, $axios, $config: { apiURL } }) {
    ...
    const projects = await $axios.$get(
      `https://d1cqjkd0k5bd3q.cloudfront.net/upload/projects-nuxt/projects_config.json`
    );

    await $axios
      .$get(`${apiURL}/instituicao/${projects[slug].id}/editais`)
      .then((response) => {
        if (response.data) {
    ...
    editais.forEach((element) => {
      $axios.$get(`${apiURL}/edital/${element.id}/pacotes`).then((response) => {
        if (response.data.length) {
          let preparatorios = [];

          if (preparatorios.length) {
            preparatorios = response.data.sort((a, b) =>
              a.attributes.cargo.localeCompare(b.attributes.cargo)
            );
          } else {
  ...
  async beforeMount() {
    await this.editais.forEach((element) => {
      this.$axios
        .$get(`${this.$config.apiURL}/edital/${element.id}/pacotes`)
        .then((response) => {
          response.data.forEach((preparatorio) => {
            this.preparatorios.push(preparatorio);
            this.$axios
              .$get(
                `${this.$config.apiURL}/preparatorio/${preparatorio.id}/arquivos`
              )
              .then((response) => {
                const cargoNome = preparatorio.attributes.cargo;
                const cargoSplit = cargoNome.split("-");
                const cargo = cargoNome.replace(
                  "-" + cargoSplit.slice(-1)[0],
                  ""
                );
    ...
    await this.$axios
      .$get(
        `${this.$config.apiURL}/edital/${this.editalMaisRecente.id}/pacotes`
      )
      .then((preparatorios) => {
        let preparatorioMaisCaro = preparatorios.data.sort((a, b) => {
          return a.attributes.valor + b.attributes.valor;
        });

        this.preparatorio = preparatorioMaisCaro[0];
      });
  },
  ...
  computed: {
    editalMaisRecente() {
      let edital = this.editais.sort((a, b) => {
        if (a.attributes.ano_edital === b.attributes.ano_edital) {
          const nEditalA =
            a.attributes.nr_edital.split("/").length > 1
              ? a.attributes.nr_edital.split("/")[1]
              : a.attributes.nr_edital.replace("Edital ", "");
          const nEditalB =
            b.attributes.nr_edital.split("/").length > 1
              ? b.attributes.nr_edital.split("/")[1]
              : b.attributes.nr_edital.replace("Edital ", "");
          return nEditalB - nEditalA;
        }
        return b.attributes.ano_edital - a.attributes.ano_edital;
      });
  ...
  methods: {
    ...
    sendNumber() {
      ...
      if (inputNumber.length >= 14 && inputNumber.length <= 15) {
        ...
        if (continuar) {
          messageStatus =
            '<h2 class="mainTextColor">Chamando um de nossos atendentes.</h2>';
          this.updateClickToCallStatus(
            messageStatus,
            "running",
            inputNumber,
            true
          ...
          this.$axios
            .$post(settings.url, settings.data, httpOptions)
            .then((response) => {
              if (!response.status) {
                if (response.response.message === "Nao Existe.") {
                  messageStatus =
                    '<h2 class="mainTextColor">Não existe. Verifique seu número.</h2>';
                }
                if (response.response.message === "Ocupado.") {
                  messageStatus =
                    '<h2 class="mainTextColor">Ocupado. Tente novamente.</h2>';
                }
                this.updateClickToCallStatus(
                  messageStatus,
                  "error",
                ...
                messageStatus =
                  '<h2 class="mainTextColor">Estamos te ligando!<h2>';
                this.updateClickToCallStatus(
                  messageStatus,
                  "success",
                  inputNumber,
          ...
          messageStatus =
            '<div class="flex flex-col"><span>Número inválido</span><h2 class="mainTextColor mt-3">Verifique o 9º dígito</h2></div>';
          this.updateClickToCallStatus(
            messageStatus,
            "error",
            inputNumber,
        ...
        messageStatus =
          '<h2 class="mainTextColor">Número inválido, tente novamente.</h2>';
